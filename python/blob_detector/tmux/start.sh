...
tmux send-keys -t "$P" "$SETUP; "'waitForGazebo; rosservice call /pairs_drone_spawner/spawn "1 $UAV_TYPE --pos -5 0 1 0.0 --enable-rangefinder --enable-mobius-camera-front"' Enter
...
# ---------------- window: edge_detector ----------------
read W_edge_detector P <<< "$(tmux new-window -t "$SESSION_NAME" -n "edge_detector" -P -F '#{window_id} #{pane_id}')"
tmux send-keys -t "$P" "$SETUP; "'waitForHw; roslaunch pairs_example_blob_detector blob_detector.launch' Enter
P=$(tmux split-window -t "$W_edge_detector" -P -F '#{pane_id}')
tmux select-layout -t "$W_edge_detector" tiled
tmux send-keys -t "$P" "$SETUP; "'waitForHw; rosrun rqt_image_view rqt_image_view /$UAV_NAME/blob_detector/processed_image/image_raw' Enter
tmux select-layout -t "$W_edge_detector" tiled
...
tmux select-window -t "$W_edge_detector"
